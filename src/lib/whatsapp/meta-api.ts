...
export interface InteractiveButton {
  ...
  title: string
  /** Button action type: quick_reply (default), url, call_phone_number, copy_code */
  type?: InteractiveButtonType
  /** URL for button of type 'url' */
  url?: string
  /** Phone number for button of type 'call_phone_number' */
  phone_number?: string
  /** Code for button of type 'copy_code' */
  code?: string
}
...
export async function sendInteractiveButtons(
  ...
  const isSingleCtaUrl =
    buttons.length === 1 &&
    buttons[0].type === 'url' &&
    Boolean(buttons[0].url?.trim())
...
// ============================================================
// Catalog & Product Messages (Meta Commerce)
// ============================================================
 ...
 * The customer sees a product card in chat with price, image, and a
 * "View" button that opens product details inside WhatsApp.
 */
export async function sendSingleProductMessage(
  ...
  if (!productRetailerId) throw new Error('Product retailer ID (SKU) is required.')
  ...
  if (!response.ok) {
    await throwMetaError(response, `Meta API error sending product: ${response.status}`)
  }
...
export interface SendMultiProductArgs {
  phoneNumberId: string
  accessToken: string
  to: string
  catalogId: string
 ...
 * Displays up to 30 products across sections.
 */
export async function sendMultiProductMessage(
  ...
  const formattedSections = sections.map((sec) => ({
    title: sec.title,
    product_items: sec.productRetailerIds.map((id) => ({
  ...
  if (!response.ok) {
    await throwMetaError(response, `Meta API error sending product list: ${response.status}`)
  }
...
/**
 * Send a Catalog Message (action name 'catalog_message').
 * Renders a "View Catalog" button that opens the merchant's full catalog.
 */
export async function sendCatalogMessage(
  ...
  if (!response.ok) {
    await throwMetaError(response, `Meta API error sending catalog: ${response.status}`)
  }
