...
describe("sendInteractiveButtons — validation", () => {
  ...
  it("sends native cta_url interactive payload when there is a single URL button", async () => {
    let captured: { url: string; body: unknown; method: string } | null = null;
    vi.stubGlobal(
      "fetch",
      vi.fn(async (url: string, init: RequestInit) => {
    ...
    const result = await sendInteractiveButtons({
      ...BASE_ARGS,
      headerText: "Check this out",
      footerText: "Tap link",
      buttons: [
        { id: "visit_site", title: "Visit Website", type: "url", url: "https://example.com" },
      ],
    });
    ...
    expect(captured!.body).toMatchObject({
      ...
      interactive: {
        type: "cta_url",
        body: { text: "Body text" },
        header: { type: "text", text: "Check this out" },
        footer: { text: "Tap link" },
        action: {
          name: "cta_url",
          parameters: {
