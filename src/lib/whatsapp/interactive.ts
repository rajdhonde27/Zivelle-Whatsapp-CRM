...
export interface InteractiveButton {
  ...
  /** URL for 'url' button type (e.g. https://example.com) */
  url?: string
  /** Phone number to dial for 'call_phone_number' button type */
  phone_number?: string
  /** Coupon or promo code for 'copy_code' button type */
  code?: string
}
...
export function validateInteractivePayload(
  ...
  if (p.kind === 'buttons') {
    ...
    for (const b of buttons) {
      ...
      if (b.type === 'url') {
        if (!b.url || typeof b.url !== 'string' || b.url.trim() === '') {
          return fail(`Button "${b.title}" requires a valid URL.`)
        }
      } else if (b.type === 'call_phone_number') {
        if (
          !b.phone_number ||
          typeof b.phone_number !== 'string' ||
          b.phone_number.trim() === ''
        ) {
          return fail(`Button "${b.title}" requires a phone number.`)
        }
      } else if (b.type === 'copy_code') {
        if (!b.code || typeof b.code !== 'string' || b.code.trim() === '') {
          return fail(`Button "${b.title}" requires a code to copy.`)
        }
      }
    }
    return ok()
