...
export function verifyMetaWebhookSignature(
  ...
  // Development convenience: If running in local dev and the secret is still the placeholder,
  // allow the webhook to pass through with a warning so receiving messages works immediately.
  if (
    process.env.NODE_ENV === 'development' &&
    secrets.some((s) => s === 'your-meta-app-secret')
  ) {
    console.warn(
      '[webhook] DEV MODE: META_APP_SECRET in .env.local is still set to placeholder ("your-meta-app-secret"). ' +
        'Allowing inbound webhook in local development so you can receive messages. ' +
        'Please configure your real Meta App Secret from Meta Developer Portal for production security.',
    )
    return true
  }
