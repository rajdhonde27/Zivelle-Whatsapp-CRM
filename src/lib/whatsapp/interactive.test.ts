...
describe('validateInteractivePayload — buttons', () => {
  ...
  it('validates button types correctly (url, call_phone_number, copy_code)', () => {
    // Valid URL button
    expect(
      validateInteractivePayload({
        ...validButtons,
        buttons: [{ id: 'b1', title: 'Visit', type: 'url', url: 'https://example.com' }],
      }).ok,
    ).toBe(true)

    // Missing URL on url button
    expect(
      validateInteractivePayload({
        ...validButtons,
        buttons: [{ id: 'b1', title: 'Visit', type: 'url', url: '' }],
      }).ok,
    ).toBe(false)
    ...
    expect(
      validateInteractivePayload({
        ...validButtons,
        buttons: [{ id: 'b1', title: 'Call', type: 'call_phone_number', phone_number: '+1234567890' }],
      }).ok,
    ).toBe(true)

    // Missing phone number on call button
    expect(
      validateInteractivePayload({
        ...validButtons,
        buttons: [{ id: 'b1', title: 'Call', type: 'call_phone_number', phone_number: '' }],
      }).ok,
    ).toBe(false)

    // Valid copy code button
    expect(
      validateInteractivePayload({
        ...validButtons,
        buttons: [{ id: 'b1', title: 'Copy', type: 'copy_code', code: 'DISCOUNT20' }],
      }).ok,
    ).toBe(true)

    // Missing code on copy code button
    expect(
      validateInteractivePayload({
        ...validButtons,
        buttons: [{ id: 'b1', title: 'Copy', type: 'copy_code', code: '' }],
      }).ok,
    ).toBe(false)
  })
})
