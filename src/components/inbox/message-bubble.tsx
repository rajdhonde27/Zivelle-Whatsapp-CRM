...
function MessageContent({
  ...
  switch (message.content_type) {
    ...
    case "order": {
      const order = message.order_details;
      const items = order?.product_items || [];
      return (
        <div className="flex flex-col gap-2 rounded-lg border border-border/50 bg-background/50 p-2.5 sm:min-w-[240px]">
              ...
              {items.map((it, idx) => (
                <div key={idx} className="flex items-center justify-between text-xs">
                  <span className="truncate text-muted-foreground">
                    <span className="font-medium text-foreground">{it.quantity}x</span>{" "}
                    {it.name || it.retailer_id}
                  </span>
                  <span className="shrink-0 font-medium">
                    {it.currency} {(it.quantity * it.item_price).toFixed(2)}
                  </span>
                </div>
              ))}
            </div>
