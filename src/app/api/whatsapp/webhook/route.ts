...
interface WhatsAppMessage {
  ...
  /** WhatsApp Product Catalog Order. */
  order?: {
    catalog_id: string
    text?: string
    product_items: Array<{
...
async function processMessage(
  ...
  const { contentText, mediaUrl, mediaType, interactiveReplyId, orderDetails } =
  ...
  // (see supabase/migrations/001_initial_schema.sql and 043_commerce_catalog.sql):
  //   conversation_id, sender_type, content_type, content_text,
  //   media_url, media_type, template_name, message_id, status,
  //   created_at, order_details

  // The messages.content_type CHECK constraint (widened in migration 043
  // to add 'order' for customer catalog orders) allows:
  //   text, image, document, audio, video, location, template, interactive, order
    ...
    .upsert(
      {
        ...
        // Only populated for content_type='order'. Migration 043.
...
async function parseMessageContent(
  ...
  /**
   * For WhatsApp Product Catalog orders submitted by customers.
   * Migration 043.
   */
  orderDetails?: OrderPayload | null
}> {
    ...
    case 'order': {
      if (message.order) {
        ...
        const itemsSummary = items
          .map(
            (i) =>
              `${i.quantity}x ${i.product_retailer_id} (${i.currency} ${i.item_price})`
          )
          .join(', ')

        const contentText = `🛒 Order received: ${itemsSummary}${
          total > 0 ? ` | Total: ${currency} ${total.toFixed(2)}` : ''
        }${ord.text ? `\nNote: ${ord.text}` : ''}`
      ...
      return { ...empty, contentText: '🛒 [Order received]' }
    }
